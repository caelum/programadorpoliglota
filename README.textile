...
Para rodar, primeiramente configure seu banco de dados em config/database.yml (há um arquivo de exemplo que pode ser seguido). Instale as gems necessárias com @bundle install@ (talvez seja necessário instalar o bundler antes). Depois faça @rake db:create@ seguido de @rake db:migrate@.
